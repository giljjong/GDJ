 ...
 *  2. Service 실행 결과가 "특정 값"인 경우 Service를 대상으로 단위 테스트를 수행할 수 있다.
 *  3. 프로젝트의 Build Path에서 JUnit 라이브러리를 추가하고 사용한다.
 *  4. 테스트 수행
 *  	프로젝트 실행 : Run - JUnit
 *  5. 주요 애너테이션
 *  	1) @Test   : 단위 테스트를 수행하는 메소드
 *  	2) @Before : 단위테스트 수행 이전에 실행하는 메소드
 *  	3) @after  : 단위테스트 수행 이후에 실행하는 메소드
 */
...
public class BoardTest {
	...
	public void 게시글삭제테스트() {
		// 삽입테스트로 삽입한 게시글 삭제
		int result = BoardDao.getInstance().deleteBoard(15);
		assertEquals(1, result);
	}
}
