 ...
 * 	Servlet
 *  1. 웹 화면을 만드는 자바 클래스
 *  2. JSP/Servlet Container(Tomcat)에 의해서 실행된다
 *  3. 서블릿을 실행하면 웹 브라우저가 실행된다
 *  4. URL
 *  	프로토콜://호스트:포트번호/콘텍스트 패스/URLMapping
...
public class HelloServlet extends HttpServlet {	// Servlet은 HttpServlet을 상속 받으면 된다
	 ...
	 * 		1) 각종 초기화 정보 실행
	 * 		2) 서블릿 환경 설정 처리
	 * 		3) init() 메소드 호출 뒤 service() 메소드 호출된다
	 */
