...
public class BindingnServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Binding
		// 1. 속성(Attribute)의 정보를 저장하고 사용하는 것
		// 2. 3개 영역을 사용한다
		// 		1) ServletContext 	  : 애플리케이션(프로젝트) 내부 어디서든 접근해서 사용 가능하다
		...
		*/
		...
		// 포워드
		// 1. request를 전달한다
		// 2. 컨텍스트 내부 이동이므로 경로 작성 시 컨텍스트 패스는 적지 않는다
		// request.getRequestDispatcher("/BindingServlet2").forward(request, response);
		
		// 리다이렉트
		// 1. request를 전달하지 않는다
		// 2. 클라이언트 -> 서버로 이동하므로 컨텍스트 패스를작성해야 한다
