...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 업로드 할 경로
		//		1) 실제 서버 경로 사용(물론 일반 들아ㅣ브 경로로 바꿀 수 있다.)
		// 		2) ServletContext의 realPath() 메소드를 이용한다.
		...
		MultipartRequest multipartRequest = new MultipartRequest(
				...
				/* 최대 크기 */	   		 1024 * 1024 * 10,
				/* 인코딩 */			 "UTF-8",
				/* 파일 명 중복 처리 */	 new DefaultFileRenamePolicy()
				);
		...
		long size = file.length();	// 바이트 단위 파일 크기
		String strSize = new DecimalFormat("#,##0").format(size);
		long lastModified = file.lastModified();	// 파일의 최종 수정일(타임 스태프)
		...
		out.println("<h3>원래 파일 명 : " + originalName + "</h3>");
		out.println("<h3>저장된 파일 명 : " + filesystemName + "</h3>");
