...
/*
    ...
    4. 트랜잭션(작업) 완료를 위해 commit이 필요하다
    5. 트랜잭션(작업) 취소를 위해 ROLLBACK 사용이 가능하다
    6. 종류
...
-- 외부 데이터 IMPORT
-- 엑셀 데이터(시트마다 테이블 1개)
-- 테이블 선택 후 마우스 우클릭 - (데이터 임폴트)

-- 부서 테이블 수정
-- 1. 부서 번호가 1인 부서의 지역을 '인천'으로 수정
UPDATE DEPARTMENT
   SET LOCATION = '인천'
 WHERE dept_no = 1;
 
-- 2. 부서 번호가 3인 부서 명을 '전략부', 지역을 '부산'으로 수정
UPDATE DEPARTMENT
   SET dept_name = '전략부'
     , location = '부산'
 WHERE DEPT_NO = 3;
 
-- 3. 부서 번호가 3인 부서의 부서 번호를 6으로 수정
-- DEPARTMENT의 부서 번호를 EMPLOYEE가 참조 중이므로 수정이 불가함
-- 해결책
-- 1. 외래키 일시 중지
-- 2. 수정
-- 3. 외래키 재시작
...
-- 1. 부서 번호가 4인 부서를 삭제
-- 부서 번호가 4인 행(ROW)을 참조하는 사원이 없으므로 정상 삭제
DELETE
  FROM DEPARTMENT
 WHERE DEPT_NO = 4;
...
-- 부서 번호가 1인 행(ROW)의 소속부서가 NULL 값으로 함께 변경
DELETE
  FROM DEPARTMENT
 WHERE DEPT_NO = 1;
