...
/*
    ...
    3. 작성된 프로시저는 EXECUTE문으로 실행
       EXECUTE 프로시저();
    4. 형식
        CREATE [OR REPLACE] PROCEDURE 프로시저_이름[(매개변수)]
        IS -- AS 가능
            변수선언
        BEGIN
            프로시저본문
...
-- 프로시저 PROC3 정의
-- 사원 번호를 전달하면 해당 사원의 FIRST_NAME을 서버메시지로 출력하기
...
-- 존재하지 않는 사원 번호는 출력 파라미터에 'NoName'반환하기
-- 프로시저 PROC6 정의
...
(
    CNO IN CUSTOMER.CUST_NO%TYPE,
    PCODE IN PRODUCT.PROD_CODE%TYPE,
    BAMOUNT IN BUY.BUY_AMOUNT%TYPE
)
IS
BEGIN
    INSERT INTO BUY(BUY_NO, CUST_NO, PROD_CODE, BUY_AMOUNT)
           VALUES (BUY_SEQ.NEXTVAL, CNO, PCODE, BAMOUNT);
    
    UPDATE CUSTOMER
       SET CUST_POINT = CUST_POINT + FLOOR(BAMOUNT * (SELECT PROD_PRICE FROM PRODUCT WHERE PROD_CODE = PCODE) * 0.1)
     WHERE CUST_NO = CNO;
    
    UPDATE PRODUCT
       SET PROD_STOCK = PROD_STOCK - BAMOUNT
     WHERE PROD_CODE = PCODE;
