...
SELECT DISTINCT B.USER_ID AS 아이디, U.USER_NAME AS 고객명, COUNT(*) AS 구매횟수, SUM(B.BUY_AMOUNT * P.PROD_PRICE) OVER(PARTITION BY B.USER_ID) AS 총구매액
  FROM PRODUCTS P INNER JOIN BUYS B
    ON P.PROD_CODE = B.PROD_CODE INNER JOIN USERS U
    ON B.USER_ID = U.USER_ID
 GROUP BY B.USER_ID, (B.BUY_AMOUNT * P.PROD_PRICE), B.BUY_AMOUNT * P.PROD_PRICE, P.PROD_PRICE, U.USER_NAME
 ORDER BY 총구매액;
...
SELECT U.USER_ID, U.USER_NAME, NVL(COUNT(*), 0)
  FROM BUYS B INNER JOIN USERS U
    ON B.USER_ID = U.USER_ID
 GROUP BY U.USER_ID, U.USER_NAME
 ORDER BY U.USER_ID;
 
SELECT U.USER_ID AS 아이디, U.USER_NAME AS 고객명, NVL((SELECT COUNT(*) FROM BUYS B WHERE B.USER_ID = U.USER_ID GROUP BY USER_ID), 0) AS 구매횟수
  FROM USERS U
 GROUP BY U.USER_ID, U.USER_NAME
 ORDER BY 아이디;
...
SELECT U.USER_NAME, P.PROD_NAME, (SUM(P.PROD_PRICE * B.BUY_AMOUNT) OVER (PARTITION BY B.BUY_NO))AS 구매액
  FROM BUYS B INNER JOIN PRODUCTS P
    ON B.PROD_CODE = P.PROD_CODE INNER JOIN USERS U
    ON B.USER_ID = U.USER_ID
...
SELECT A.USER_NAME, A.CNT
  FROM (SELECT U.USER_NAME, COUNT(*) AS CNT
          FROM BUYS B INNER JOIN USERS U
            ON B.USER_ID = U.USER_ID
...
SELECT P.PROD_NAME, NVL((SELECT COUNT(*) FROM BUYS B WHERE B.PROD_CODE = P.PROD_CODE GROUP BY PROD_CODE), 0) AS 판매횟수
  FROM PRODUCTS P;
...
SELECT U.USER_NAME AS 고객명, NVL((SELECT P.PROD_NAME FROM PRODUCTS P WHERE P.PROD_CODE = B.PROD_CODE), 'NULL') AS 구매제품
