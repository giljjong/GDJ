...
BEGIN
    SELECT FIRST_NAME, LAST_NAME, SALARY INTO F_NAME, L_NAME, V_SALARY
      FROM EMPLOYEES
...
BEGIN
    ...
    END IF;
     ...
     DBMS_OUTPUT.PUT_LINE(F_NAME || L_NAME || '님의 연봉 ' || V_SALARY || '는 ' || RESULT || '입니다');
END;
...
-- FIRST_NAME 과 LAST_NAME을 레코드 변수에 저장하고 해당 값을 조회하기
...
BEGIN
    ...
    WHILE EMP_NO <= 206 LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(FULL_NAME.V_FNAME || ' ' || FULL_NAME.V_LNAME);
        EMP_NO := EMP_NO + 1;
    END LOOP;
END;
...
BEGIN
    ...
    END LOOP;
        AG := TOTAL / CNT;
    DBMS_OUTPUT.PUT_LINE('전체 합계는 ' || TOTAL || ' 이고, 평균은 ' || AG);
END;
...
BEGIN
    ...
    FOR EMP_ID IN 100..206 LOOP
        ...
        TOTAL := TOTAL + SAL;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('연봉 합계 : ' || TOTAL);
END;
