...
/*
    ...
    3. 서브쿼리는 메인 쿼리에 괄호()를 이용해서 포함시킴
    4. 항상 서브쿼리를 먼저 실행하고, 서브쿼리 실행 결과를 메인 쿼리에서 사용한다
    5. 사용되는 절에 따른 구분
        1) SELECT절 : 스칼라 서브쿼리
        2) FROM절   : 인라인뷰
        3) WHERE절  : 서브쿼리
...
-- 1. 사원 번호가 1001인 사원과 같은 직급(POSITION)을 가진 사원조회하기
...
-- 2. 가장 급여(SALARY)가 높은 사원 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE SALARY = (SELECT MAX(SALARY)
                   FROM EMPLOYEE);          -- 서브쿼리가 함수이므로 단일 행 서브쿼리 연산자(=) 사용
                   
-- 3. 부서번호가 1인 부서와 같은 지역에 있는 부서를 조회하시오
...
-- 4. 평균 급여 이상을 받는 사원 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE SALARY >= (SELECT FLOOR(AVG(SALARY))
                   FROM EMPLOYEE);

-- 5. 평균 근속 기간 이상을 근무한 사원 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY, FLOOR(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) AS 근속기간
  FROM EMPLOYEE
...
-- 9. 부서 번호가 1인 부서에 근무하는 사원의 급여보다 더 많은 급여를 받는 사원 조회하기
...
-- 10. 부서 번호가 1인 부서에 근무하는 사원의 급여보다 더 많은 급여를 받는 사원 조회하기
-- 모든 급여(2000000, 5000000)이든 하나의 급여보다 많이 받으면 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
...
-- 1. 전체 사원의 인원 수, 급여 합계/평균/최대/최소 조회하기
SELECT 
...
-- 2. 부서 번호가 1인 부서와 같은 지역에서 근무하는 사원 조회하기
...
-- 따라서 스칼라 서브쿼리와 동일한 방식의 조인은 외부 조인이다
SELECT E.EMP_NO
     , E.NAME
     , E.DEPART
     , (SELECT D.DEPT_NAME
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFBqAABAAALDBAAC';
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWNUM BETWEEN 1 AND 3; -- 조회가능
...
-- ROWNUM에 별명을 지정하고 해당 별명을 사용한다
-- 3 : SELECT
-- 1 :   FROM (ROWNUM의 별명 지정하기)
-- 2 :  WHERE ROWNUM의 별명 사용하기
SELECT R.ROW_NUM, R.EMP_NO, R.NAME
...
SELECT A.ROW_NUM, A.EMP_NO, A.NAME, A.SALARY
...
SELECT A.EMP_NO, A.NAME, A.SALARY
 ...
 SELECT A.ROW_NUM, A.EMP_NO, A.NAME, A.HIRE_DATE
