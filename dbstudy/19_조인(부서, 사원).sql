...
/*
    ...
    4. 종류
        ...
        3) 외부 조인
            - OUTER JOIN
            - 한 테이블은 일치하는 행을 조인하고, 한 테이블은 일치 여부와 상관없이 일치하지 않아도 조인한다
...
*/
-- 1. 크로스 조인 확인
SELECT E.EMP_NO, NAME, E.SALARY, E.DEPT_NO, D.DEPT_NAME
...
--    모두 포함 시킬 사원 테이블을 OUTER JOIN의 왼쪽/오른쪽에 두느냐에 따라
--    왼쪽 외부 조인/오른쪽 외부 조인으로 구분함

-- DRIVE/DRIVEN 테이블이 잘못 지정된 조인
SELECT E.EMP_NO, E.NAME, D.DEPT_NAME
  FROM EMPLOYEE E LEFT OUTER JOIN DEPARTMENT D  -- 왼쪽의 EMPLOYEE 테이블은 모두 조회하시오
    ON E.DEPART = D.DEPT_NO;

-- DRIVE/DRIVEN 테이블이 제대로 지정된 조인
SELECT E.EMP_NO, E.NAME, D.DEPT_NAME
  FROM DEPARTMENT D RIGHT OUTER JOIN EMPLOYEE E -- 오른쪽의 EMPLOYEE 테이블은 모두 조회하시오
    ON  D.DEPT_NO = E.DEPART;