...
INSERT INTO SAMPLE2(NAME, KOR, ENG, MATH) VALUES(NULL, 100, 100, 100);
INSERT INTO SAMPLE2(NAME, KOR, ENG, MATH) VALUES('영숙', NULL, 100, 100);
INSERT INTO SAMPLE2(NAME, KOR, ENG, MATH) VALUES('정수', 100, NULL, 100);
INSERT INTO SAMPLE2(NAME, KOR, ENG, MATH) VALUES('지영', 100, 100, NULL);
COMMIT;

-- NULL 값이 연산에서 사용되면 결과가 NULL이다.
SELECT 1 + NULL FROM DUAL;

-- NULL 처리 함수
-- 1. NVL 함수
--    NVL(칼럼, 칼럼 값이 NULL일 때 대신 사용할 값)
...
-- 집계함수 (그룹 함수)
-- 1. 통계(합계, 평균, 최대, 최소, 갯수 등)를 낼 때 사용
-- 2. NULL 값을 연산에서 제외
-- 3. 종류
--    1) SUM(칼럼) : 칼럼 합계
--    2) AVG(칼럼) : 칼럼 평균
...
SELECT
     ...
     , SUM(ENG)     -- 영어 합
     , SUM(MATH)    -- 수학 합
--     , SUM(KOR, ENG, MATH)     인수(arguments)가 3개 이므로 불가능 함.(인수 1개여야 함)
