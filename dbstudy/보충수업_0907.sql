...
SELECT O.ORDER_ID AS 구매번호, C.CUSTOMER_NAME AS 구매자, B.BOOK_NAME AS 책이름, B.PRICE AS 판매가격, O.ORDER_DATE AS 주문일자
  FROM ORDERS O INNER JOIN CUSTOMER C
    ON O.CUSTOMER_ID = C.CUSTOMER_ID INNER JOIN BOOK B
    ON O.BOOK_ID = B.BOOK_ID
 WHERE TO_CHAR(O.ORDER_DATE, 'YYYY/MM/DD') < '2014/07/04'
   OR TO_CHAR(O.ORDER_DATE, 'YYYY/MM/DD') > '2014/07/07';
...
SELECT DISTINCT C.CUSTOMER_NAME AS 고객명, SUM(B.PRICE * O.AMOUNT) OVER(PARTITION BY C.CUSTOMER_NAME) AS 총구매액
  FROM ORDERS O INNER JOIN BOOK B
    ON O.BOOK_ID = B.BOOK_ID INNER JOIN CUSTOMER C
...
SELECT C.CUSTOMER_NAME AS 고객명,
       NVL((SELECT DISTINCT SUM(B.PRICE * O.AMOUNT) OVER(PARTITION BY C.CUSTOMER_NAME) FROM ORDERS O, BOOK B WHERE O.BOOK_ID = B.BOOK_ID AND O.CUSTOMER_ID = C.CUSTOMER_ID), 0) AS 총구매액,
       NVL((SELECT COUNT(ORDER_ID) FROM ORDERS O WHERE C.CUSTOMER_ID = O.CUSTOMER_ID GROUP BY CUSTOMER_NAME), 0) AS 구매횟수
  FROM CUSTOMER C
...
SELECT DISTINCT C.CUSTOMER_NAME AS 고객명, COUNT(B.PUBLISHER) AS 출판사수
  FROM ORDERS O INNER JOIN BOOK B
    ON O.BOOK_ID = B.BOOK_ID INNER JOIN CUSTOMER C
    ON O.CUSTOMER_ID = C.CUSTOMER_ID
...
SELECT A.고객명  
 FROM (SELECT CUSTOMER_NAME AS 고객명,
              (SELECT COUNT(*) FROM ORDERS O WHERE C.CUSTOMER_ID = O.CUSTOMER_ID GROUP BY CUSTOMER_NAME) AS B
        FROM CUSTOMER C) A
WHERE B IS NULL;
...
SELECT A.고객명, A.책이름, A.주문일자
  FROM (SELECT C.CUSTOMER_NAME AS 고객명, B.BOOK_NAME AS 책이름, O.ORDER_DATE AS 주문일자
          FROM CUSTOMER C INNER JOIN ORDERS O
...
-- 고객명  책이름       책가격
-- NULL    골프 바이블  35000
SELECT (SELECT C.CUSTOMER_NAME AS 고객명 FROM CUSTOMER C WHERE C.CUSTOMER_ID = O.CUSTOMER_ID) AS 고객명, B.BOOK_NAME AS 책이름, B.PRICE AS 책가격
...
SELECT B.고객명, B.총구매액
  FROM (SELECT ROWNUM AS ROW_NUM, A.고객명, A.총구매액
               FROM (SELECT DISTINCT C.CUSTOMER_NAME AS 고객명,
                            SUM(B.PRICE * O.AMOUNT) OVER(PARTITION BY C.CUSTOMER_NAME) AS 총구매액
                       FROM CUSTOMER C INNER JOIN ORDERS O
                       ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK B
                       ON O.BOOK_ID = B.BOOK_ID
                       ORDER BY 총구매액 DESC) A)B
