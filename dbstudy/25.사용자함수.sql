/*
    ...
    5. 입력 파라미터 사용 가능(출ㄹ겨 파라미터 사용 안함)
...
RETURN VARCHAR2         -- 반환 타입에서는 크기를 명시하지 않음
IS
BEGIN
...
-- 함수 FUNC2 정의
-- 사원 번호를 전달하면 해당 사원의 FULL_NAME이 반환되는 함수
...
IS
    RESULT NUMBER;
BEGIN
    RESULT := CEIL(N * POWER(10, DIGIT) / POWEWR(10, DIGIT);
    RETURN RESULT;
END;
