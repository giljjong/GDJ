...
/*
    타입 변환 함수
    1. TO_NUMBER('문자열')         : 문자열 형식의 숫자를 숫자 형식으로 변환한다
    2. TO_CHAR(데이터, '형식')     : 지정된 데이터(주로 숫자나 날짜)를 형식에 맞는 문자열로 변환한다
    3. TO_DATE('문자열', '형식')   : 지정된 문자열을 날짜 형식으로 변환한다.
*/
...
-- 지정된 형식으로 해석
-- 예시 1) 'YY/MM/DD' : 05년 06월 07일
-- 예시 2) 'MM/DD/YY  : 07년 05월 06일
...
SAMPLE(DT1, DT2, DT3)
VALUES
(SYSDATE, SYSTIMESTAMP, TO_CHAR(SYSDATE, 'YYYY-MM-DD'));
