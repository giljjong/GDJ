	...
	public static void main(String[] args) {
		...
		while(unit1.isAlive() && unit2.isAlive()) {		// while(unit1.getEnergy() > 0 && unit2.getEnergy() > 0)
			if(myTurn) {
				...
				System.out.println(unit1.getName() +"의 공격력 : " + unit1.getPower());
				System.out.println(unit2.getName() + "의 남은 체력 : " + unit2.getEnergy());
				System.out.println();
			} else {
				System.out.println(unit2.getName() + "의 공격! ");
				unit2.attack(unit1);		// unit1이 unit2를 공격한다. (마린은 40% 확률로 상대를 한 번에 죽인다)
				System.out.println(unit2.getName() +"의 공격력 : " + unit2.getPower());
				System.out.println(unit1.getName() + "의 남은 체력 : " + unit1.getEnergy());
				System.out.println();
			}
			myTurn = !myTurn;
		}
